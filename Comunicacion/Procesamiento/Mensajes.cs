...
namespace AlarmaApi.Comunicacion.Procesamiento
{
    internal class Mensajes
    {
        ...
        public static void HeartBeat(string idDispositivo)
        {
            ...
            if(Procesos.HeartBeat.timeOuts.Where(x => x.Key.idDispositivo == item.Key.idDispositivo).FirstOrDefault().Key != null)
            {
                ...
                if(item.Key.estadoConexion != Models.Dispositivo.ESTADO_CONEXION.CONECTADO.ToString())
                    MongoDB.GetInstance().InsertarNotificacionDispositivoConectado(item.Key);
            }
        }
